...
const useAudio = url => { const [audio] = useState(new Audio(url)); 
...
const toggle = () => setPlaying(!playing); 
    ...
    useEffect(() => { audio.addEventListener('ended', () => setPlaying(false));
        ...
        }, []); return [playing, toggle]; }; const Player = ({ url }) => { 
            const [playing, toggle] = useAudio(url);

        return ( <div> 
            
            <button onClick={toggle}>{playing ? "Pause "+url : "Play "+url}</button> </div> ); 
